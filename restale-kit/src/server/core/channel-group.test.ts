...
describe('client context storage and control propagation', () => {
  type PushContext = { page: number; sortBy?: string }
  type ScopedGroup = SSEChannelGroup<SWRSignal, TestMeta, 'swr', PushContext>

  function createGroup(): ScopedGroup {
    return new SSEChannelGroup<SWRSignal, TestMeta, 'swr', PushContext>({ target: 'swr' })
  }

  it('stores context for every channel on a connection and exposes the stored value', async () => {
    const group = createGroup()
    const first = createSSEChannel({ target: 'swr', connectionId: 'ctx-shared-connection' })
    const second = createSSEChannel({ target: 'swr', connectionId: 'ctx-shared-connection' })
    ...
    expect(group['channels'].get(first)?.clientContext).toEqual({ page: 2, sortBy: 'createdAt' })
  ...
  it('requires a supplied scope to match trusted channel metadata before writing context', async () => {
    const group = createGroup()
    const channel = createSSEChannel({ target: 'swr', connectionId: 'ctx-scoped-connection' })
    group.register(channel, { userId: 100, role: 'admin' })

    await expect(group.updateClientContext(channel.connectionId, { page: 4 }, { scope: { userId: 999 } }))
      .resolves.toEqual({ updated: false })
    expect(group.getClientContext(channel.connectionId)).toBeUndefined()

    await expect(group.updateClientContext(channel.connectionId, { page: 4 }, { scope: { userId: 100 } }))
      .resolves.toEqual({ updated: true })
    expect(group.getClientContext(channel.connectionId)).toEqual({ page: 4 })
  })

  it('validates context before applying it and retains the previous value after a rejected update', async () => {
    const schema = createInvalidSchema<PushContext>('Invalid client context')
    const group = new SSEChannelGroup<SWRSignal, TestMeta, 'swr', PushContext>({
      target: 'swr',
      clientContextSchema: schema,
    })
  ...
  it('stores the normalized value returned by clientContextSchema', async () => {
    const schema = createValidSchema<PushContext>(() => ({ page: 1, sortBy: 'updatedAt' }))
    const group = new SSEChannelGroup<SWRSignal, TestMeta, 'swr', PushContext>({
      target: 'swr',
      clientContextSchema: schema,
    })
  ...
  it('propagates through the control topic while leaving non-owning instances unchanged', async () => {
    const pubsub = new MemoryPubSubAdapter<SWRSignal>()
    const owner = new SSEChannelGroup<SWRSignal, TestMeta, 'swr', PushContext>({ target: 'swr', pubsub })
    const nonOwner = new SSEChannelGroup<SWRSignal, TestMeta, 'swr', PushContext>({ target: 'swr', pubsub })
    const channel = createSSEChannel({ target: 'swr', connectionId: 'ctx-cluster-owner' })
    owner.register(channel, { userId: 100 })
    await owner['controlPendingOp']
    await nonOwner['controlPendingOp']

    await owner.updateClientContext(channel.connectionId, { page: 3 }, { scope: { userId: 100 } })

    expect(owner.getClientContext(channel.connectionId)).toEqual({ page: 3 })
    expect(nonOwner.getClientContext(channel.connectionId)).toBeUndefined()
    await owner.dispose()
    await nonOwner.dispose()
  })

  it('drops context when the final channel for a connection deregisters', async () => {
    const group = createGroup()
