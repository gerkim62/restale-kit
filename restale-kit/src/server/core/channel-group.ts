...
> {
   ...
   * criteria-based matching. To revoke such channels, use `revokeByConnectionId(connectionId)` instead.
   */
   ...
   * a single compromised token, or removing a specific session without touching others.
   *
   * ## Security: always pass `scope`
   *
   * `connectionId` is generated by the client and sent as a URL query parameter — it is an opaque
   * correlation value, NOT an authentication credential. Any HTTP client can fabricate or replay
   * an arbitrary connection ID. Without `scope`, a forged ID could revoke someone else's connection.
   *
   * Always combine `connectionId` with trusted server-side identity via `scope`:
   *
   * ```ts
   * // ✅ Safe: scope pins the lookup to the authenticated user's connections only
   * await group.revokeByConnectionId(connectionId, { userId: req.user.id })
   *
   * // ✅ Even tighter: add sessionId for per-session precision
   * await group.revokeByConnectionId(connectionId, { userId: req.user.id, sessionId: req.session.id })
   *
   * // ❌ Unsafe: no scope — any connectionId can close any connection
   * await group.revokeByConnectionId(req.body.connectionId)
   * ```
   *
   * `scope` is subset-matched against the channel's registered metadata using structural deep
   * equality — the same semantics as `revokeWhere`. If the channel's metadata does not contain
   * every key-value pair in `scope`, the revocation is rejected and `{ closed: false }` is returned.
   * This is intentional: a mismatch means the supplied ID does not belong to the requesting user.
   *
   * If a pub/sub adapter is configured, a control message is broadcast to the cluster so that remote
   * instances can close the matching connection even if it is not held locally.
   */
