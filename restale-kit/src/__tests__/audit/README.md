...
- **`frame-guard.test.ts`**: Tests that `beforeFrame` intercepts real `Signal` payloads for signal frames, receives undefined signal on keepalive ticks, skips `EventStore` persistence when returning `skip`, and enqueues revoke frames on `close`.
