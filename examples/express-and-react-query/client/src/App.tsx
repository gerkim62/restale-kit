...
function Dashboard() {
  ...
  // Connect to ReStale SSE stream
  const { connection, reconnect } = useReStale<AppSignal>(
    userId ? `/api/sse?userId=${userId}` : '',
    {
      disabled: !userId,
  ...
  const createTodoMutation = useMutation({
    ...
    onSuccess: (newTodo) => {
      ...
      qc.setQueryData<Todo[]>(['todos', { userId }], (prev = []) => [...prev, newTodo]);
    },
  });
  ...
  const toggleTodoMutation = useMutation({
    mutationFn: async ({ id, completed }: { id: string; completed: boolean }) => {
      const res = await fetch(`/api/todos/${id}?userId=${userId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
    ...
    onSuccess: (updatedTodo) => {
      qc.setQueryData<Todo[]>(['todos', { userId }], (prev = []) =>
