...
- **Open question:** whether a receiving instance re-runs `clientContextSchema` against a value arriving via the control topic, or trusts it as already-validated at the origin instance (mirroring how the value was validated once before publish). Not yet decided — resolve before implementing §13.3's cross-instance test.
