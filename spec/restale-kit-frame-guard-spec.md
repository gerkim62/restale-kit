...
- Synchronous exceptions thrown by the guard function should be handled inside the function itself when possible (e.g. deciding whether an error during a state lookup should fail open or fail closed). Because `beforeFrame` is strictly synchronous, it cannot time out itself — any timeout handling belongs at the separate upstream component boundary (e.g., async auth/session refresh background services). If an unhandled synchronous error is thrown inside `beforeFrame`, restale-kit catches it, logs a warning, and fails closed by treating it as `{ action: 'close' }` to ensure security invariants are preserved.
